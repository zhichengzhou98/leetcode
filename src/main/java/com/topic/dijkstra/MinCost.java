...
/**
 * @author zc.zhou
 * @Description 1928. 规定时间内到达终点的最小花费 TODO: 解答错误
...
public class MinCost {
  ...
  private void dijkstra(List<int[]>[] lists, int begin, int[] passingFees, int maxTime) {
    ...
    pq.offer(new int[]{begin, distances[0], 0});
    while (!pq.isEmpty()) {
      int[] current = pq.poll();
      int node = current[0];
      int fee = current[1];
      ...
      for (int[] next : nexts) {
        ...
        if (nextDis + fee >= distances[nextNode] || curTime + time > maxTime) {
          continue;
        }
        //当前路线到nextNode的距离更近，更新此距离，并把nextNode放入优先队列
        distances[nextNode] = nextDis + fee;
        pq.offer(new int[]{nextNode, distances[nextNode], curTime + time});
      }
    }
  }
}
